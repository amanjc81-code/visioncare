...
# ---------------------------------------------------------------------------
# Test 8 — Grade range (must be 0, 1, 2, or 3)
# ---------------------------------------------------------------------------

def test_grade_range():
    """Test that the output grade is always one of {0, 1, 2, 3}."""
    sample = _ensure_sample_exists()
    _get_or_make_checkpoint()
    ...
    assert grade in {0, 1, 2, 3}, f"Grade must be 0-3, got {grade}"
...
def test_output_keys_exact():
    ...
    result = predict(str(sample))
    required_keys = {"grade", "probabilities", "gradcam_path"}
...
def test_probability_keys():
    """Test that probabilities dict has exactly keys '0', '1', '2', '3'."""
    sample = _ensure_sample_exists()
    _get_or_make_checkpoint()
    ...
    assert set(probs.keys()) == {"0", "1", "2", "3"}, (
        f"Probability keys must be '0','1','2','3', got {set(probs.keys())}"
    )
